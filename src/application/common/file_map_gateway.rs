...
pub trait FileMapReader {
    async fn get_file(&self, article_id: &ArticleId, id: &FileId) -> Result<Option<File>, FileMapGatewayError>;
}

#[async_trait]
pub trait FileMapWriter {
    async fn save_file(&self, article_id: &ArticleId, file: &File) -> Result<(), FileMapGatewayError>;
}

#[async_trait]
pub trait FileMapRemover {
    async fn remove_file(&self, article_id: &ArticleId, file_id: &FileId) -> Result<(), FileMapGatewayError>;
}

#[async_trait]
pub trait FileMapArticle {
...
pub trait FileMapGateway: FileMapReader + FileMapWriter + FileMapRemover + FileMapArticle {}
