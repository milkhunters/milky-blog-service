...
async def update_comment(
    ...
    Пользователь с ролью ADMIN.ONE может обновить чужой комментарий
    """
...
async def delete_comment(comment_id: uuid.UUID, service: ServiceFactory = Depends(get_services)):
    ...
    Пользователь с ролью ADMIN.ONE может удалить чужой комментарий
    """
...
async def delete_all_comments(article_id: uuid.UUID, service: ServiceFactory = Depends(get_services)):
    ...
    Только Администратор(>=ADMIN.ONE) может удалить все комментарии статьи
    """
